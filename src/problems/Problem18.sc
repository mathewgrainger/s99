...
val data: List[Symbol] = List('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k)

def slice[T](i : Int, k: Int, xs : List[T]) = xs.drop(i).take(k - i)
slice(3, 7, data)

def slice2[T](i : Int, k: Int, xs : List[T]) = xs.slice(i, k)
slice2(3, 7, data)

def sliceF[T](i : Int, k: Int, xs : List[T]) =
  xs.zipWithIndex.filter((a) => a._2 >= i && a._2 < k).map((a) => a._1)
sliceF(3, 7, data)

/* official answer

I didn't do the recursive answers - and missed an error case in my take/drop answer.

lessons learnt
1. there is a slice method
2.
